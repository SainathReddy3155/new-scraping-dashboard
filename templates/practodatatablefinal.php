...
<body class="fixed-nav sticky-footer bg-light" id="page-top">
  ...
  <div class="card-body">
    ...
    <form id="tweetsearch" method="POST">
      <b>Keywords : </b><input type="text" name="keywords" class="practokeyword" id="practokeywords"  placeholder="Enter Keywords for aggregator" required>
      <button type="Submit" class="btn btn-outline-primary practokeywordbtn" id="practosearchbtn" >Get Data</button>
</form>
    ...
    <div id="loader"  style="display:none;">
      ...
      <div class="row">
        ...
        <div class="col-lg-6 col-sm-6">
            <button class="btn btn-primary downloadallpracto"><i class="fa-sharp fa-solid fa-download"></i> Download  excel</button>
        </div>
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form method="POST" action="/practosavechanges">
                      

                      <div class="mb-3">
                        <label for="Practo Name" class="form-label"><b>Aggregator Name</b></label>
                        <input type="text" class="form-control" id="PractoName" name="practoname" aria-describedby="Practo Name" readonly>
                      </div>
                      <div class="mb-3">
                        <label for="Specialization" class="form-label"><b>Specialization</b></label>
                        <input type="text" class="form-control" id="Specialization" name="speciality" aria-describedby="Specialization" >
                      </div>
                      <div class="mb-3">
                        <label for="Experience" class="form-label"><b>Experience</b></label>
                        <input type="text" class="form-control" id="Experience" name="experience" aria-describedby="Experience" >
                      ...
                      <div class="mb-3">
                        ...
                        <textarea type="text" class="form-control" id="Address" name="address" aria-describedby="Location" ></textarea>
                      </div>
...
<div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="mb-3">
                        <label for="Practo Name" class="form-label"><b>Permanently Delete Aggregator Name</b></label>
                        <input type="text" class="form-control" id="practonamedelete" readonly name="practoname"  aria-describedby="deleteuser" >
                      </div>
      <div class="mb-3">
...
<script>
    $('#example').on('click', 'td.editor-edit', function (e) {
        ...
        var td=tr.find("td")
        var practoName=$(td[0]).text()
        var Specialization=$(td[1]).text()
        var Experience=$(td[2]).text()
        var Location=$(td[3]).text()
        var Address=$(td[4]).text()
...
<script>
    $('#example').on('click', 'td.editor-delete', function (e) {
        ...
        var name=$(td[0]).text()
        console.log(name)
        $("#practonamedelete").attr("value",name)
...
<script>
    $(document).ready(function(){
        $("#practosearchbtn").on("click",function(){
            if($("#practokeywords").val()==""){
                alert("Please enter keywords")
            }
            else{
                var keywords=$("#practokeywords").val();
                $.ajax({
                    url:'/practoscraping',
                    type:'POST',
                    data:JSON.stringify({'data':keywords}),
                    contentType:"application/json",
                    beforeSend:function(){
                      $("#loader").show();
                      $("#practosearchbtn").prop("disabled",true)
                    },
                    success:function(response){
                        console.log("success")
                        var successmessage=response['success']
                        $("#successmessage").html("✅ "+successmessage).delay(5000).fadeOut()
                    },
                    complete:function(){
                        $("#loader").hide();
                        // $("#successmessage").html("We have suuccessfully fetached your data")
                        $("#practosearchbtn").prop("disabled",false)
                        $("input").val("");
                        },
                    error:function(){
...
<script>
  $(document).ready(function(){
    $(".downloadallpracto").on("click",function(){
      window.location.replace("/downloadallaggregator")
    });
  });
</script>
  ...
  $(document).ready(function(){
    $("#modaldelete").on("click",function(){
      if(($("#deleting").val()==="") || ($("#deleting").val()!="Delete")) {
          ...
          var name=$("#practonamedelete").val();
          var inputdata=$("#deleting").val();
          console.log("name is :",name)
          console.log("inputdata : ",inputdata)
          $.ajax({
            url:'/practodeletedata',
            type:'POST',
            data:JSON.stringify({'practoname':name,'inputdata':inputdata}),
            contentType:'application/json',
            success:function(response){
              // console.log("success")
