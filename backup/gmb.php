    ...
    <div id="loader"  style="display:none;">
      ...
      <div class="row">
        ...
        <div class="col-lg-6 col-sm-6 col-md-6 searchbtn d-flex">
            <b>Search :</b> <input type="text" required placeholder="Search" name="searchtext" id="gmbsearchid">&nbsp;
     ...
     <center>
      ...
      <table class="table mt-8 responsive gmbtable" >
        <thead>
            <tr>
            <th scope="col">Gmb Name</th>
            <th scope="col">Gmb_Rating</th>
            <th scope="col">Total_Reviews</th>
...
<span class="fw-bold">Current Page:<span class="currentpage" id="cpc">0</span></span>
<!--editmodal-->
...
<script>
  $(document).ready(function(){
    ...
    var pglength=pgno.length
    console.log("length : ",pglength)
    if (pglength<3){
      var currentpgno=pgno.split("")[0]
      $(".currentpage").html(currentpgno)
      console.log(currentpgno)
    }
    else if(pglength>3){
      var currentpgno=pgno.split("")

      for (let i=0;i<currentpgno.length;i++){
          var currentpgno=currentpgno[0]+currentpgno[1]+currentpgno[2]
          $(".currentpage").html(currentpgno)
          console.log("greater than 4  :",currentpgno)
      }
    }
    else{
      var currentpgno=pgno.split("")
      ...
      for (let i=0;i<currentpgno.length;i++){
          var currentpgno=currentpgno[0]+currentpgno[1]
          $(".currentpage").html(currentpgno)
          console.log("sadfnkldjaf :",currentpgno)
      }
...
<script>
  $(document).ready(function(){
      var urlerrorpgno="{{variable.urlerror}}"
      // console.log("urlerror : ",urlerrorpgno)
      var lenofresults="{{variable.lenofresults}}"
      // console.log("len of resulsts : ",lenofresults)
      if (urlerrorpgno){
        window.location.replace("/gmb?pgno=0")
      }
...
<script>
  $(document).ready(function(){
    // var currentpgno=$("#cpc").val();
    // console.log("currentpgno",currentpgno)
    $("#previous").on("click",function(){
      ...
      if(pgno=='0'){
        ...
        window.location.replace('/gmb?pgno='+pgno)
      }
      ...
      success:function(response){
          ...
          var modal=`<form method="POST" action="/GmbSavechanges">
                      <div class="mb-3">
                        ...
                        <textarea id="gmbname" name="gmbname" readonly rows="4" cols="50">`+gmbname+`</textarea>
                      </div>
...
<script>
  $(document).ready(function(){
      $("#gmbsearchid").on("keyup",function(){
        var searchvalue=$("#gmbsearchid").val();
        console.log(searchvalue)
        $.ajax({
        url:'/gmbsearchdata',
        type:'POST',
        data:JSON.stringify({'gmbsearchvalue':searchvalue}),
        contentType:'application/json',
        success:function(response){
